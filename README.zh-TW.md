...
Trend: ADX: 26.1 ▲ | Aroon: 85 ▲
Volume: VWAP: $67,287 ▲ | VWMA: $67,100 ▲
Money: CMF: +0.12 ▲ | MFI: 59.6 ▲
🗣️ 1H: 📉 Down 震盪偏空
...
價格後方會依條件顯示 🟢（買進/反彈）或 🔴（賣出/回落）。判斷分兩步驟，**依序**檢查：

1. **ADX(14) > 20**（前提條件，濾掉盤整行情）——不成立就不顯示任何 emoji，直接跳過步驟 2。
2. **RSI(14)** 決定方向：
   - `RSI(14) < 30`（超賣極端值） → 🟢 買進/反彈
   - `RSI(14) > 70`（超買極端值） → 🔴 賣出/回落
   - 介於 30~70 之間 → 不顯示

對應程式碼：`scripts/send_tg_notification.py` 的 `_signal_emoji()`。

> **為什麼是這個規則**：舊版用「Keltner 突破 + VWAP/VWMA + EMA200 + RSI>50」4 選 3，
> 經 [`docs/indicator-backtest/`](docs/indicator-backtest/README.md) 用 2026-02~09 的 BTC/ETH
> 24H 前瞻回測驗證後，兩幣種兩個方向的 edge 全部是負的（比瞎猜還差）。改用 RSI(14) 逆勢極端值
> 搭配 ADX>20 濾網後，🔴 賣出訊號在 BTC/ETH 都有穩定正 edge（+5~+13pp），🟢 買進訊號在 ETH
> 同樣可靠（+7.8pp），但 **BTC 的 🟢 買進訊號本身可預測性偏弱（edge 接近 0）**——這是這段資料
> 期間 BTC 超賣後不見得會在 24H 內反彈的市場特性，不是規則設計的問題，出現 BTC 🟢 時請自行加判斷。

#### EMA 均線

顯示 EMA 20 / 50 / 100 / 200 四組均線，**最靠近當前價格**的那組加粗顯示。

```
EMA: 20 $2,351 | 50 $2,335 | 100 $2,319 | 200 $2,262
     ↑ 粗體（離當前價格最近）
```
...
| `DC(S)` | 停損導向動態支撐 = Donchian Lower(20) × 緩衝（加密 ×0.97 / 金屬 ×0.98）。詳見 [交易策略說明](docs/trading-strategies.md) |
...
K 線型態是由 1 根或數根 K 棒（蠟燭圖）的形狀組合所形成的反轉或持續訊號。顯示在 `Oscillator:` 下方，無型態時整行隱藏。
...
| 型態 | 結構 | 出現位置 | 意義 |
|------|------|---------|------|
| **三白兵** | 連續 3 根長紅K，每根收盤高於前根 | 下跌後 | 最強多頭確認，買方完全主導 |
| **早晨之星** | 長黑K → 小實體K → 長紅K | 底部 | 三根K底部反轉，賣方力竭 |
| **多頭吞噬** | 大紅K完全包住前根黑K | 跌勢中 | 買方突然接管，強烈反轉訊號 |
| **錘形** | 小實體在上，下影線 ≥ 2倍實體 | 底部 | 跌勢中出現，買方在低點大力買入 |
| **倒錘形** | 小實體在下，上影線 ≥ 2倍實體 | 底部 | 底部出現，買方嘗試拉升但收回，仍為潛在反轉 |

**空頭反轉型態（▼）**

| 型態 | 結構 | 出現位置 | 意義 |
|------|------|---------|------|
| **三黑鴉** | 連續 3 根長黑K，每根收盤低於前根 | 上漲後 | 最強空頭確認，賣方完全主導 |
| **黃昏之星** | 長紅K → 小實體K → 長黑K | 頂部 | 三根K頂部反轉，買方力竭 |
| **空頭吞噬** | 大黑K完全包住前根紅K | 漲勢中 | 賣方突然接管，強烈反轉訊號 |
| **射擊之星** | 小實體在下，上影線 ≥ 2倍實體 | 頂部 | 漲勢中出現，買方拉升後遭賣壓打回 |
| **上吊線** | 小實體在上，下影線 ≥ 2倍實體 | 頂部 | 頂部出現，外形像錘形但位置在高點，警示反轉 |

> **新手提示：** 型態訊號搭配其他指標確認時更可靠。例如：錘形出現時，若 CMF 同步轉正、RSI < 30，反轉可信度更高。單獨的型態僅供參考。

#### ADX 趨勢強度與方向

`ADX` 顯示趨勢強度，方向由 +DI / -DI 決定：
...
ADX > 50 時數值**粗體**顯示（強趨勢）。

#### Aroon 趨勢方向

`Aroon` 顯示趨勢的新鮮度（0–100）。只顯示主導方向的值：

| 方向 | 顯示值 | 意義 |
|------|--------|------|
| `▲`（Aroon Up > Down） | Aroon Up 值 | 越高代表越近期創高，多頭趨勢越強 |
| `▼`（Aroon Down > Up） | Aroon Down 值 | 越高代表越近期創低，空頭趨勢越強 |
| `(=)` | 較高者 | 兩者接近，方向不明 |

#### ADX + Aroon 雙重確認

ADX 方向（▲/▼）與 Aroon 方向一致時，兩者**粗體**顯示，代表趨勢訊號獲得雙重確認：

```
ADX: 26.1 ▲ | Aroon: 85 ▲    ← 同向，粗體（強確認）
ADX: 26.1 ▲ | Aroon: 80 ▼    ← 分歧，不粗體
ADX: 18.3 (=) | Aroon: 40 (=) ← 無趨勢
```
...
`Volume:` 行顯示以成交量加權的價格指標，▲/▼ 代表 price 在指標上方或下方。同向時兩者**粗體**。

| 欄位 | 說明 |
|------|------|
| `VWAP` | 成交量加權平均價 — 機構參考價格，price > VWAP = ▲ 多方，price < VWAP = ▼ 空方 |
| `VWMA` | 成交量加權移動均線(20) — 帶量均線，與 VWAP 同向時兩者**粗體**顯示 |

#### Money（資金流向）

`Money:` 行顯示資金流入/流出指標，▲/▼ 代表資金方向。同向時兩者**粗體**。

| 欄位 | 說明 |
|------|------|
| `CMF` | Chaikin Money Flow — 資金淨流向。> 0 = ▲ 流入，< 0 = ▼ 流出 |
| `MFI` | 資金流量指數 — 帶成交量的 RSI。> 50 = ▲，< 50 = ▼。< 20 / > 80 超賣/超買時獨立**粗體** |

```
Volume: VWAP: $67,287 ▲ | VWMA: $67,100 ▲    ← 同向粗體
Money: CMF: +0.12 ▲ | MFI: 59.6 ▲            ← 同向粗體
```
...
本專案的 Telegram 通知背後運行三個經回測驗證的策略。**完整規則、回測數據與風險控制見 [交易策略說明](docs/trading-strategies.md)**；策略如何被驗證見 [復盤機制](docs/review-mechanism.md)。

| 策略 | 一句話 | 回測勝率 |
|------|--------|:---:|
| **DC(S) 停損支撐** | `Donchian Lower × 緩衝`（加密 ×0.97 / 金屬 ×0.98），依資產波動度分開校準 | 盤中守住 90-98% |
| **Fibonacci S1 多單入場（1️⃣）** | 價格進入 S1 區間時的長多進場，入場/停損/止盈盈虧比 3:1 | ETH 91% / BTC 90% |
| **Squeeze 蓄力突破** | BB 收縮進 KC 內側 = 波動極低蓄力，方向性突破預警 | 方向預警 |

> ⚠️ 以上為技術指標的歷史統計，非投資建議，請自負盈虧。
