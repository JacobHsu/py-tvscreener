"""
Send TradingView Daily XAU/XAG technical notification to Telegram.
...
Usage:
    python scripts/send_metals_notification.py          # send to Telegram
    python scripts/send_metals_notification.py --dry-run # print only, don't send
"""
...
def _get_ma_streak(conn, symbol: str, current_signal: str) -> int:
    """Count consecutive days symbol has had the given ma_rating_signal."""
    rows = conn.execute(
...
def get_latest_data() -> dict:
    """Return latest XAU & XAG rows from SQLite as {symbol: dict}."""
    ...
    cur = conn.execute(
        ...
        "WHERE symbol LIKE 'COMEX:%' ORDER BY collected_at DESC LIMIT 1"
    )
    row = cur.fetchone()
    if not row:
        conn.close()
...
def predict_with_ai(symbol_short: str, d: dict) -> dict | None:
    """Call NVIDIA NIM API (Llama 3.3 70B) for daily prediction."""
    ...
    prompt = (
        f"You are a precious metals technical analyst. Analyze the following {symbol_short} "
        f"technical indicators (DAILY timeframe) and predict the direction for the NEXT TRADING DAY.\n\n"
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a professional precious metals technical analyst. Be concise and direct.",
            },
...
def format_symbol_block(d: dict, emoji: str, symbol_short: str, pred: dict | None) -> str:
    ...
    if ma_sig == "Strong Buy":
        ma_sig_display = f"🔥 {ma_sig} ×{ma_streak}D"
    ...
    if pred:
        ...
        lines.append(
            f'🗣️ Daily: {pred_emoji} <b>{pred["prediction"]} {pred["style"]}</b>'
        )
        lines.append(f'{bull_bear}: {pred["reason"]}')
    else:
        lines.append("🗣️ Daily: ⚪ AI 預測暫不可用")
...
def build_message(data: dict, predictions: dict) -> str:
    ...
    parts = [f"📅 {time_str} 貴金屬日線 (台北時間)"]
    parts.append("")

    gold = data.get("COMEX:GC1!")
    if gold:
        parts.append(format_symbol_block(gold, "🥇", "XAU", predictions.get("XAU")))
        parts.append("")

    silver = data.get("COMEX:SI1!")
    if silver:
        parts.append(format_symbol_block(silver, "🥈", "XAG", predictions.get("XAG")))

    parts.append("")
    parts.append(
        "📊 <a href='https://tw.tradingview.com/markets/futures/quotes-metals/'>TradingView Metals</a>"
    )
    return "\n".join(parts)
...
def main():
    ...
    predictions = {}
    for symbol, key in [("XAU", "COMEX:GC1!"), ("XAG", "COMEX:SI1!")]:
        d = data.get(key)
        if d:
