"""
Collect BTC/ETH 4H technical indicators from TradingView and store in SQLite.

Usage:
    python scripts/collect_4h_to_db.py
"""
...
def build_fields():
    ...
    return [
        ...
        # Note: CryptoField has no _240 candle pattern fields — skipped
    ]
...
CREATE TABLE IF NOT EXISTS {TABLE} (
    ...
    -- Ratings (4H) — numeric + human-readable signal
...
CREATE_INDEX_SQL = f"""
CREATE UNIQUE INDEX IF NOT EXISTS idx_4h_symbol_time
    ON {TABLE}(symbol, collected_at);
"""
...
def init_db():
    ...
    conn.commit()
    conn.close()
    log.info("Database ready: %s (table: %s)", DB_PATH, TABLE)
...
def collect_and_store():
    ...
    log.info("Querying TradingView 4H for %s ...", SYMBOLS)
    df = cs.get()
